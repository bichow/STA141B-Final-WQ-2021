...
'covid %>%
  filter(Country == "Afghanistan")' # peeking at the data
...
server <- function(input, output, session){
  ...
  output$barPlot <- renderPlot({
    ...
    'title <- sprintf(
      ...
      input$country)'
    barplot(c(covid_data()$TotalConfirmed, covid_data()$NewConfirmed, covid_data()$TotalDeaths, 
  ...
  output$summary <- renderTable({
    req(input$country != "-")
    covid_data() %>%
      summarize("Confirmed Cases" = TotalConfirmed, "New Confirmed Cases" = NewConfirmed,"Deaths" = TotalDeaths,
  ...
  output$confirm_table <- renderTable({
   covid %>%
      summarize("Total Confirmed Cases" = sum(TotalConfirmed), "Median Confirmed Cases" = floor(median(TotalConfirmed)),
                "New Confirmed Cases" = sum(NewConfirmed), "Median New Confirmed Cases" = floor(median(NewConfirmed)))
    
  },width = 800)
  output$death_table <- renderTable({
    covid %>%
      summarize(" Total Deaths" = sum(TotalDeaths), " Median Deaths" = floor(median(TotalDeaths)),
                " New Deaths" = sum(NewDeaths), " Median New Deaths" = floor(median(NewDeaths)))
    
  }, width = 800)
  output$recover_table <- renderTable({
    covid %>%
      summarize("Total Recovered Cases" = sum(TotalRecovered), "Median Recovered Cases" = floor(median(TotalRecovered)),
